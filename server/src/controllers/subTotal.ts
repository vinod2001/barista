...
export const getSubTotal = (req: Request, res: Response) => {
    try {
        ...
        let tax = 0;
        let grandTotal = 0
        let discount = checkDiscount(combosAddedToCart);
        ...
        res.send({ subTotal: total, salesTax: tax, discountPercentage: discount, discount: totalDiscountAmout, grandTotal: grandTotal });
    } catch (error) {
