...
export const createCart = (req: Request, res: Response) => {
                ...
                for (const b in items.lists) {
                    ...
                    for (const i in comboLists) {
                        ...
                        for (const c in comboData) {
                            if (comboData[c].item === items.lists[b].name && items.lists[b].id === cart[s].id) {
                                ...
                                combosAddedToCart.push({ name: comboData[c].item, id: items.lists[b].comboId });
                            }
                        }
                    }
                    // creating cart list
    ...
    res.status(200).send({ totalCartItems: cartItems.length, combosAddedToCart: combosAddedToCart, message: "Added to cart succesfully" })
}
